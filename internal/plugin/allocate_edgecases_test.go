...
func TestMultipleReservationsOverflow(t *testing.T) {
    ...
    if w1.Code != 200 { t.Fatalf("reserve1 failed: %d", w1.Code) }
    ...
    if w2.Code != 200 { t.Fatalf("reserve2 failed: %d", w2.Code) }

    p.statusMux.RLock()
    rem := p.deviceRemaining["dev0"]
    p.statusMux.RUnlock()
    if rem != 0 { t.Fatalf("expected remaining 0 after overflow reserves, got %d", rem) }

    p.allocationsMux.RLock()
    a1 := p.allocations["ns/p1"]["dev0"]
    a2 := p.allocations["ns/p2"]["dev0"]
    p.allocationsMux.RUnlock()
    if a1 != 30 { t.Fatalf("expected p1 alloc 30, got %d", a1) }
    if a2 != 20 { t.Fatalf("expected p2 alloc 20, got %d", a2) }
    ...
    if uw.Code != 200 { t.Fatalf("unreserve p1 failed: %d", uw.Code) }

    p.statusMux.RLock()
    rem2 := p.deviceRemaining["dev0"]
    p.statusMux.RUnlock()
    if rem2 != 30 { t.Fatalf("expected remaining 30 after unreserve p1, got %d", rem2) }
...
func TestPartialAllocateConsume(t *testing.T) {
    ...
    if w.Code != 200 { t.Fatalf("reserve failed: %d", w.Code) }

    // Allocate only dev0 -> should consume only dev0 entry
    if _, err := p.getAllocateResponse([]string{"dev0"}); err != nil { t.Fatalf("allocate failed: %v", err) }

    p.allocationsMux.RLock()
    allocMap := p.allocations["ns/px"]
    p.allocationsMux.RUnlock()
    if allocMap == nil { t.Fatalf("expected remaining allocation entries for pod") }
...
func TestDoubleReserveSamePod(t *testing.T) {
    ...
    p.allocationsMux.RUnlock()
    if alloc != 35 { t.Fatalf("expected accumulated alloc 35, got %d", alloc) }
