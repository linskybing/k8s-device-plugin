...
func MergeReservationIntoNodeState(node NodeReservation, res scheduler.Reservation) (NodeReservation, error) {
	spec := res.Spec
	...
	// Collect candidate device indices which can accept the requested percent.
	...
	// Pick the first N candidate devices (simple heuristic).
...
func RemoveReservationFromNodeState(node NodeReservation, res scheduler.Reservation) (NodeReservation, error) {
	spec := res.Spec
